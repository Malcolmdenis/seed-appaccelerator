...
export class GitHubService {
    ...
    private static async modifyReadmeForGitHub(fs: MemFS, githubRepoUrl: string): Promise<void> {
        try {
            ...
            if (!content.includes('[cloudflarebutton]') && !content.includes('[deploybutton]')) {
                GitHubService.logger.info('README.md has no deploy button placeholder');
                return;
            }

            const modified = content
                .replaceAll('[cloudflarebutton]', '')
                .replaceAll('[deploybutton]', '');
            ...
            await git.commit({
                fs,
                dir: '/',
                message: 'docs: Add deploy info to README',
                author: {
                    name: 'seed-bot',
                    email: 'bot@seed.ar',
