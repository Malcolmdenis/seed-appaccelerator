    ...
    */
        ...
        try {
            ...
            try {
                ...
                if (wranglerConfigFile.success) {
                    await env.SeedStore.put(this.getWranglerKVKey(instanceId), wranglerConfigFile.content);
    ...
    async deployToCloudflareWorkers(instanceId: string, target: DeploymentTarget = 'platform'): Promise<DeploymentResult> {
        ...
        try {
            ...
            const wranglerConfigContent = await env.SeedStore.get(this.getWranglerKVKey(instanceId));
            ...
            // Step 3a: Collect any additional worker modules that the dispatched
            // worker will need at runtime.
            //
            // Two distinct categories:
            //
            // 1. Vite-emitted dynamic-import chunks (e.g. userRoutes-<hash>.js).
            //    These live next to index.js (or in nested subfolders).
            //
            // 2. Source files imported via `await import(/* @vite-ignore */ ...)`.
            //    Vite does NOT bundle these (the @vite-ignore directive opts out).
            //    The vibesdk vite-reference template uses this pattern with
            //    `./user-routes` so users can drop in their own routes file.
            //    For these to work at runtime in Cloudflare Workers, we must
            //    locate the source TS file, compile it, and upload it as a
            //    sibling module to index.js. (Otherwise the deploy succeeds,
            //    the worker boots, and the first /api/* request 500s with
            //    'No such module "user-routes"'.)
            //
            // Strategy:
            //   a) Scan the worker bundle directory recursively for .js files
            //      (covers Vite-emitted chunks at any depth).
            //   b) Fallback: scan the legacy `dist/assets/` location used by
            //      older builds.
            //   c) For any `await import(/* @vite-ignore */ '<spec>')` in the
            //      worker bundle, look for a matching compiled source file in
            //      `dist/<envName>/` or, if absent, find the source .ts in the
            //      sandbox's `worker/` directory and emit a transpiled JS
            //      sibling module so the runtime import resolves.
            let additionalModules: Map<string, string> | undefined = new Map<string, string>();
            try {
                // usedPath is e.g. /workspace/<instanceId>/dist/<workerName>/index.js
                const usedPathSandboxRel = (usedPath ?? '').replace(/^\/workspace\//, '');
                const lastSlash = usedPathSandboxRel.lastIndexOf('/');
                const workerBundleDir = lastSlash >= 0 ? usedPathSandboxRel.slice(0, lastSlash) : '';
                const workerBundleFileName = lastSlash >= 0 ? usedPathSandboxRel.slice(lastSlash + 1) : usedPathSandboxRel;

                // (a) Recursively scan the worker bundle directory for *.js
                if (workerBundleDir) {
                    this.logger.info('Scanning worker bundle dir for sibling modules', { workerBundleDir });
                    const findResult = await this.safeSandboxExec(
                        `find ${workerBundleDir} -type f -name "*.js" ! -name "${workerBundleFileName}"`
                    );
                    if (findResult.exitCode === 0) {
                        const paths = findResult.stdout.trim().split('\n').filter((p: string) => p.trim());
                        for (const fullPath of paths) {
                            const moduleName = fullPath.replace(workerBundleDir + '/', '');
                            try {
                                const buf = await this.readFileAsBase64Buffer(fullPath);
                                additionalModules.set(moduleName, buf.toString('utf8'));
                                this.logger.info('Worker sibling module loaded', { moduleName });
                            } catch (e) {
                                this.logger.warn(`Failed to read worker sibling module ${fullPath}`, e);
                            }
                        }
                    }
                }

                // (b) Legacy fallback: dist/assets/ (older builds without @cloudflare/vite-plugin)
                const legacyAssetsPath = `${instanceId}/dist/assets`;
                const legacyCheck = await this.safeSandboxExec(`test -d ${legacyAssetsPath} && echo y || echo n`);
                if (legacyCheck.exitCode === 0 && legacyCheck.stdout.trim() === 'y') {
                    const findLegacy = await this.safeSandboxExec(`find ${legacyAssetsPath} -type f -name "*.js"`);
                    if (findLegacy.exitCode === 0) {
                        const paths = findLegacy.stdout.trim().split('\n').filter((p: string) => p.trim());
                        for (const fullPath of paths) {
                            const moduleName = fullPath.replace(`${instanceId}/dist/`, '');
                            if (additionalModules.has(moduleName)) continue;
                            try {
                                const buf = await this.readFileAsBase64Buffer(fullPath);
                                additionalModules.set(moduleName, buf.toString('utf8'));
                                this.logger.info('Legacy assets module loaded', { moduleName });
                            } catch (e) {
                                this.logger.warn(`Failed to read legacy module ${fullPath}`, e);
                            }
                        }
                    }
                }

                // (c) Find every `import(/* @vite-ignore */ <spec>)` in the worker
                //     bundle. For each spec that's a relative path with no matching
                //     compiled sibling in additionalModules, transpile the source TS
                //     and add it as a sibling module the runtime can resolve.
                const ignoreRe = /import\(\s*\/\*\s*@vite-ignore\s*\*\/\s*['"`]([^'"`]+)['"`]/g;
                const specs = new Set<string>();
                for (const m of workerContent.matchAll(ignoreRe)) {
                    const raw = m[1];
                    // Strip query/hash bust suffix (e.g. ./user-routes?t=123)
                    const clean = raw.split('?')[0].split('#')[0];
                    specs.add(clean);
                }
                if (specs.size > 0) {
                    this.logger.info('Detected @vite-ignore dynamic imports in worker bundle', {
                        specs: Array.from(specs),
                    });
                }
                for (const spec of specs) {
                    if (!spec.startsWith('./') && !spec.startsWith('../')) continue;
                    const baseName = spec.replace(/^\.\//, '').replace(/^\.\.\//, '');
                    const candidates = [`${baseName}.js`, baseName, `${baseName}/index.js`];
                    const alreadyResolved = candidates.some((c) => additionalModules!.has(c));
                    if (alreadyResolved) {
                        this.logger.info('Vite-ignore spec already resolved by sibling scan', { spec });
                        continue;
                    }
                    // Try to locate the source TS file under <instance>/worker/
                    const tsCandidates = [
                        `${instanceId}/worker/${baseName}.ts`,
                        `${instanceId}/worker/${baseName}.tsx`,
                        `${instanceId}/worker/${baseName}/index.ts`,
                        `${instanceId}/worker/${baseName}/index.tsx`,
                    ];
                    let foundSrcPath: string | null = null;
                    for (const candidate of tsCandidates) {
                        const test = await this.safeSandboxExec(`test -f ${candidate} && echo y || echo n`);
                        if (test.exitCode === 0 && test.stdout.trim() === 'y') {
                            foundSrcPath = candidate;
                            break;
                        }
                    }
                    if (!foundSrcPath) {
                        this.logger.warn('No source file found for @vite-ignore spec — runtime import will 500', {
                            spec,
                            tsCandidates,
                        });
                        continue;
                    }
                    // Transpile the source TS to JS using the sandbox's bun (already used for build).
                    // bun build produces a single ESM file we can ship as a sibling module.
                    const outName = `${baseName}.js`;
                    const tmpOut = `${instanceId}/.viberoute_tmp_${baseName.replace(/[^a-z0-9]/gi, '_')}.js`;
                    const buildCmd = `bun build ${foundSrcPath} --target=browser --format=esm --outfile=${tmpOut} 2>&1`;
                    const buildResult = await this.safeSandboxExec(buildCmd);
                    if (buildResult.exitCode !== 0) {
                        this.logger.warn('bun build for @vite-ignore source failed', {
                            spec,
                            foundSrcPath,
                    ...
                    try {
                        const buf = await this.readFileAsBase64Buffer(tmpOut);
                        additionalModules.set(outName, buf.toString('utf8'));
                        this.logger.info('Bundled @vite-ignore source into deploy', {
                            spec,
                            moduleName: outName,
                            sourceFile: foundSrcPath,
                        });
                    } catch (e) {
                        this.logger.warn('Failed to read transpiled @vite-ignore output', { tmpOut, e });
                    } finally {
                        await this.safeSandboxExec(`rm -f ${tmpOut}`);
                    }
                }
                ...
                if (additionalModules.size === 0) {
                    ...
                    this.logger.info('Bundling additional worker modules into deploy', {
                ...
                if (additionalModules && additionalModules.size === 0) additionalModules = undefined;
            }
