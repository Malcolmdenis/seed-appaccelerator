    ...
    */
    ...
    async deployToCloudflareWorkers(instanceId: string, target: DeploymentTarget = 'platform'): Promise<DeploymentResult> {
        ...
        try {
            ...
            // Step 3a: Collect any additional worker modules emitted alongside
            // the main bundle (e.g. dynamic-import chunks like `userRoutes-<hash>.js`
            // generated by Vite when the template uses `await import('./userRoutes')`).
            //
            // The previous implementation looked in `dist/assets/` — but @cloudflare/vite-plugin
            // emits worker chunks NEXT TO index.js, not in dist/assets/. Result:
            // index.js was uploaded but the userRoutes chunk was missing, so the
            // deployed worker boots fine and 500s on first /api/* request with
            // 'No such module "user-routes"'. Scan the worker bundle's actual
            // directory and upload every *.js file (except index.js itself).
            let additionalModules: Map<string, string> | undefined;
            try {
                // usedPath is e.g. /workspace/<instanceId>/dist/<workerName>/index.js
                // Strip the leading /workspace/ that the sandbox FS strips off the
                // path it accepts in `find` and `readFileAsBase64Buffer`.
                ...
                if (workerBundleDir) {
                    this.logger.info('Scanning for additional worker modules', { workerBundleDir });

                    // Use maxdepth 1 — chunks live next to index.js, not in nested dirs
                    const findResult = await this.safeSandboxExec(
                        `find ${workerBundleDir} -maxdepth 1 -type f -name "*.js" ! -name "${workerBundleFileName}"`
                    );
                    ...
                    if (findResult.exitCode === 0) {
                        ...
                        if (modulePaths.length > 0) {
                            ...
                            for (const fullPath of modulePaths) {
                                // Cloudflare's module resolver matches against the
                                // basename used in the import specifier. Vite rewrites
                                // `await import('./userRoutes')` to `import('./userRoutes-<hash>.js')`,
                                // so registering by basename is what the runtime expects.
                                const moduleName = fullPath.split('/').pop() ?? fullPath;

                                try {
                                    const buffer = await this.readFileAsBase64Buffer(fullPath);
                                    const moduleContent = buffer.toString('utf8');
                                    additionalModules.set(moduleName, moduleContent);
                            ...
                            this.logger.info('No additional worker module chunks found next to index.js');
                        }
                    }
                }
            } catch (error) {
