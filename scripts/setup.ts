...
class SetupManager {
	...
	constructor() {
		console.log('Seed Development Setup');
		console.log('======================\n');
	}
	...
	private async collectUserConfig(): Promise<void> {
		...
		if (useAIGateway) {
			...
			const gatewayName = wranglerConfig.vars?.CLOUDFLARE_AI_GATEWAY || 'seed-gateway';
	...
	private async ensureKVNamespace(binding: string): Promise<{ id: string; title: string }> {
		const namespaceName = `seed-${binding.toLowerCase()}-local`;
	...
	private async ensureAIGateway(resources: ResourceInfo): Promise<void> {
		const gatewayName = this.config.useAIGateway ? 'seed-gateway' : null;
	...
	private displayFinalReport(report: ReadinessReport, resources: ResourceInfo): void {
		...
		console.log('\nSetup complete. The App Accelerator is ready.');
	}
