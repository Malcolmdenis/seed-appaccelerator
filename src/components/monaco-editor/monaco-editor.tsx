...
monaco.editor.defineTheme('seed-dark', {
	base: 'vs-dark',
	inherit: true,
	rules: [
...
monaco.editor.defineTheme('seed', {
	base: 'vs',
	inherit: true,
	rules: [
...
export const MonacoEditor = memo<MonacoEditorProps>(function MonacoEditor({
	...
	useEffect(() => {
		...
		editor.current = monaco.editor.create(containerRef.current!, {
			...
			theme: configuredTheme === 'dark' ? 'seed-dark' : 'seed',
	...
	useEffect(() => {
		if (editor.current) {
			monaco.editor.setTheme(theme === 'dark' ? 'seed-dark' : 'seed');
		}
	}, [theme]);
