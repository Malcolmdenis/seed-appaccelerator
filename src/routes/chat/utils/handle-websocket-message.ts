...
export interface HandleMessageDeps {
    ...
    setProductionDeploymentUrl: React.Dispatch<React.SetStateAction<string>>;
    ...
    return (websocket: WebSocket, message: WebSocketMessage) => {
        ...
        switch (message.type) {
            ...
            case 'production_deployment_started': {
                setIsDeploying(true);
                sendMessage(createAIMessage('production_deployment_started', message.message));
                break;
            }
            ...
            case 'production_deployment_completed': {
                ...
                setProductionDeploymentUrl(message.deploymentUrl);
                setDeploymentError('');
                setIsRedeployReady(false);

                sendMessage(createAIMessage('deployment_completed', `Your project has been permanently deployed: ${message.deploymentUrl}`));
