...
function DebugPanelCore({ messages, onClear, chatSessionId }: DebugPanelProps) {
  ...
  const calculateOperationMetrics = (wsMessages: DebugMessage[]) => {
    try {
      ...
      // Track production deployment: production_deployment_started → production_deployment_completed
      const prodStarts = wsMessages.filter(m => m.messageType === 'production_deployment_started');
      const prodCompletes = wsMessages.filter(m => m.messageType === 'production_deployment_completed');
      
      prodStarts.forEach(start => {
        const complete = prodCompletes.find(c => c.timestamp > start.timestamp);
        if (complete) {
          operations.prodDeployment.push(complete.timestamp - start.timestamp);
        }
      });
      ...
      return {
        ...
        prodDeployment: getStats(operations.prodDeployment),
      ...
      return {
        ...
        prodDeployment: { avg: 0, median: 0, p99: 0, count: 0 },
  ...
  const categorizeWebSocketMessage = useCallback((messageType?: string): 'generation' | 'phase' | 'file' | 'deployment' | 'system' | undefined => {
    ...
    // Deployment messages
    if (['production_deployment_started', 'production_deployment_completed', 'production_deployment_error', 'deployment_completed'].includes(messageType)) {
      return 'deployment';
    }
  ...
  const emailDump = async () => {
    ...
    try {
      ...
      const mailtoLink = `mailto:support@seed.ar?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body + '\n\n' + JSON.stringify(dump, null, 2))}`;
                      ...
                      {Object.entries(analyticsData.operations)
                        ...
                        .map(([operation, stats]) => {
                          ...
                          const operationConfig = {
                            ...
                            prodDeployment: { icon: '☁️', color: 'orange', label: 'Production Deployment' },
