...
export function createAgentConnection(
		...
		try {
			...
			switch (parsed.type) {
				...
				case 'production_deployment_error':
					emitter.emit('production', parsed);
					break;
				case 'error':
	...
	async function connectNow(): Promise<void> {
		...
		if (typeof WebSocket === 'undefined') {
			emitter.emit('ws:error', {
				error: new Error(
					'WebSocket is not available. This SDK requires a runtime with native WebSocket support (Edge Workers, browsers, Bun, or Node.js 22+).'
				),
			});
			return;
		}
