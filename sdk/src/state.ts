...
export type ProductionDeploymentState =
	| { status: 'idle' }
...
export class SessionStateStore {
	...
	applyWsMessage(msg: AgentWsServerMessage): void {
		switch (msg.type) {
			...
			case 'production_deployment_started': {
				const m = msg as WsMessageOf<'production_deployment_started'>;
				this.setState({ production: { status: 'running', instanceId: m.instanceId } });
				break;
			}
			case 'production_deployment_error': {
				const m = msg as WsMessageOf<'production_deployment_error'>;
				this.setState({
					production: { status: 'failed', error: m.error, instanceId: m.instanceId },
				});
				break;
			}
			case 'production_deployment_completed': {
				const m = msg as WsMessageOf<'production_deployment_completed'>;
				this.setState({
					production: {
						status: 'complete',
