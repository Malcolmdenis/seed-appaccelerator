...
export class BuildSession {
	...
	readonly wait = {
		...
		productionDeployed: (options: WaitOptions = {}) => this.waitForProductionDeployed(options),
	};
	...
	async waitForProductionDeployed(
		options: WaitOptions = {},
	): Promise<WsMessageOf<'production_deployment_completed'>> {
		const timeoutMs = options.timeoutMs ?? this.getDefaultTimeoutMs();
		const msg = await this.waitForWsMessage(
			(m) =>
				m.type === 'production_deployment_completed' || m.type === 'production_deployment_error',
			timeoutMs,
		);
		if (msg.type === 'production_deployment_error') {
			throw new Error((msg as WsMessageOf<'production_deployment_error'>).error);
		}
		return msg as WsMessageOf<'production_deployment_completed'>;
	}
